<script lang="ts">
	...
	import {
		clearManualQueue,
		getQueueTrackPosition,
		reorderManualQueue,
		reorderQueue
	} from "$lib/scripts/queue"
...
{#if $queueReady}
	<div
		...
		class:expanded={$queueBoxVisible}
	>
		<div class="body">
			<div class="list">
				<div class="label">
				...
				{#each [$currentTrack] as track (track.id)}
					<div
						in:receive={{ key: track.id }}
						out:send={{ key: track.id }}
			...
			{#if hasManual}
				<div transition:fade={{ duration: 200 }} class="list">
					<div class="label">
						<span class="text"> MANUALLY QUEUED </span>
						<button class="btn" on:click={() => ($queueStage = clearManualQueue())}>
							<span class="tip"> CLEAR QUEUE </span>
					...
					{#each manual as track (track.id)}
						...
						>
							<QueueItem
								...
								id={track.id}
								position="MANUAL"
								on:skiphere={skipToTrack}
								on:removefromqueue={removeFromQueue}
							/>
						</div>
					{/each}
				</div>
...
<style lang="scss">
	...
	.queue-box {
		...
		transition:
			...
			min-width $hover-fade-time ease;
