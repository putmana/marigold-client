<script lang="ts">
	...
	function unshuffleTracks() {
		if (playingTrack.source === "AUTO") {
			...
			if (pos != -1) {
				...
				console.warn(
					"Could not find a track with ID " +
						playingTrack.id +
						" (" +
	...
	function processStage() {
		...
		if ($queueStage.flag === "START") {
			...
			prevTracks = $queueStage.tracks
				.slice(0, $queueStage.position)
				.filter((track) => track.source === "AUTO")
			console.log(prevTracks)
			nextTracks = $queueStage.tracks
				.slice($queueStage.position + 1, $queueStage.tracks.length)
				.filter((track) => track.source === "AUTO")
			manualTracks = $queueStage.tracks
				.slice($queueStage.position + 1, $queueStage.tracks.length)
				.filter((track) => track.source === "MANUAL")
			playingTrack = $queueStage.tracks[$queueStage.position]
			reload()
...
<div class="scrub">
	<ScrubBar bind:currentTime={time} bind:duration disabled={!ready} on:timechange={handleScrub} />
</div>
<div class="main">
	<div class="info">
		<div class="art">
	...
	<div class="settings" style="--full-light: {$playerPalette.bright.light}">
		<button
			class="btn selected"
			on:click={toggleQueueBox}
			class:selected={$queueBoxVisible}
			disabled={!ready}
		>
			<div class="btn-icon">
